	...
	mutations: {
		...
		insertMessage(state, msgInfo) {
			...
			if (!msgInfo.selfSend && msgInfo.status != MESSAGE_STATUS.READED) {
				// chat.unreadCount++;
				chat.unreadCount = msgInfo.num;
			}
			// 如果是群消息
      if (msgInfo.groupId&&msgInfo.type!=21) {
        const unreadObj = msgInfo.map;
        const curUser = JSON.parse(uni.getStorageSync("zhixin_applets_loginInfo"));
        for (const key in unreadObj) {
          if (unreadObj.hasOwnProperty.call(unreadObj, key)) {
            if (curUser && curUser.sUserId == key) {
              chat.unreadCount = unreadObj[key];
            }
          }
        }
      }
		...
		updateChatFromGroup(state, group) {
			for (let i in state.chats) {
				...
				if (chat.type == 'GROUP' && chat.targetId == group.id) {
					...
					chat.showName = group.remark?group.remark:group.name;
					break;
				}
			}
			this.commit("saveToStorage");
		...
		saveToStorage(state) {
			let userId = userStore.state.userInfo.userId;
		...
		setOnlineStatus(state, onlineIds) {
			state.chats.forEach((c) => {
				const online = onlineIds.find((id) => c.targetId == id);
				if (online) {
					c.online = true;
				} else {
					c.online = false;
